/**
 * Logika tombol "Not Yet" lari
 */
function pindahTombol() {
    const btn = document.getElementById('btnEngga');
    const daftarPesan = ["nt mell! 😜", "ngeyel! 🙄", "coba lagi! 😋", "pencet iyaa aja gasi!? 😡", "dih maksa! 😭", "gabisa mel sori! 🤪"];
    btn.innerText = daftarPesan[Math.floor(Math.random() * daftarPesan.length)];
    btn.style.position = 'fixed';
    const maxX = window.innerWidth - btn.offsetWidth - 50;
    const maxY = window.innerHeight - btn.offsetHeight - 50;
    btn.style.left = Math.random() * maxX + 'px';/**
 * Logika tombol "Not Yet" lari dan ganti teks secara acak
 */
function pindahTombol() {
    const btn = document.getElementById('btnEngga');
    const daftarPesan = [
        "nt mell! 😜", "ngeyel! 🙄", "coba lagi! 😋",
        "pencet iyaa aja gasi!? 😡", "dih maksa! 😭",
        "gabisa mel sori! 🤪", "sekali lagi mell! 😜"
    ];

    // Ganti teks tombol secara acak
    const pesanAcak = daftarPesan[Math.floor(Math.random() * daftarPesan.length)];
    btn.innerText = pesanAcak;

    // Pindahkan posisi tombol (agar tidak keluar layar iPhone 11 Pro Max)
    btn.style.position = 'fixed';
    const maxX = window.innerWidth - btn.offsetWidth - 50;
    const maxY = window.innerHeight - btn.offsetHeight - 50;

    const randomX = Math.random() * maxX;
    const randomY = Math.random() * maxY;

    btn.style.left = randomX + 'px';
    btn.style.top = randomY + 'px';
}

/**
 * Animasi mengetik teks
 */
async function typeWriter(id, text, speed = 40) {
    const el = document.getElementById(id);
    for (let i = 0; i < text.length; i++) {
        el.innerHTML += text.charAt(i);
        await new Promise(res => setTimeout(res, speed));
    }
}

/**
 * Fungsi untuk reload halaman (Replay)
 */
function lokasiReload() {
    window.location.reload();
}

/**
 * FUNGSI UTAMA - Dijalankan saat tombol "Yes" diklik
 */
async function terimaMaaf() {
    const music = document.getElementById('bgMusic');
    const scene = document.getElementById('dramatic-scene');
    const videoWrapper = document.querySelector('.video-wrapper-landscape');
    const videoEl = document.getElementById('us-video');
    const replayScreen = document.getElementById('replay-screen');

    // 1. Jalankan Musik (Mulai dari detik 210 / 3:30)
    music.currentTime = 210;
    music.volume = 0;
    music.play();
    
    // Efek Fade In Musik (Biar nggak kaget)
    let fadeIn = setInterval(() => {
        if (music.volume < 0.7) {
            music.volume += 0.05;
        } else {
            clearInterval(fadeIn);
        }
    }, 200);

    // 2. Monitoring Waktu Lagu (Selesai di detik 276)
    music.ontimeupdate = () => {
        if (music.currentTime >= 276) { 
            finishEverything();
        }
    };

    // Fungsi untuk menutup momen dan munculkan Replay
    function finishEverything() {
        scene.classList.add('fade-out-all');
        let fadeOut = setInterval(() => {
            if (music.volume > 0.02) {
                music.volume -= 0.02;
            } else {
                music.pause();
                clearInterval(fadeOut);
                scene.style.display = 'none';
                replayScreen.style.display = 'flex';
                setTimeout(() => { replayScreen.style.opacity = '1'; }, 100);
            }
        }, 100);
    }

    // 3. Transisi Visual (Meredupkan Card Utama)
    document.getElementById('content-wrapper').style.opacity = '0';
    document.getElementById('bg-decorations').style.opacity = '0';
    
    setTimeout(() => {
        document.getElementById('content-wrapper').style.display = 'none';
        document.getElementById('bg-decorations').style.display = 'none';
        scene.style.display = 'flex'; // Munculkan scene video
    }, 800);

    // 4. Memunculkan Video & Mulai Mengetik
    await new Promise(r => setTimeout(r, 2000)); 
    videoWrapper.classList.add('show-video');
    videoEl.playbackRate = 0.8; // Video diperlambat agar sinematik
    videoEl.play();

    // Jalankan urutan teks puitis
    await typeWriter("type1", "In the world of literature, there are countless beautiful verses, but none can truly capture how much you mean to me.");
    await new Promise(r => setTimeout(r, 800));
    
    await typeWriter("type2", "Just like the lyrics in your photo, 'Lights will guide you home'...");
    await new Promise(r => setTimeout(r, 500));
    
    await typeWriter("type3", "I hope I can be one of those lights that always leads you back to where you feel safe and comfortable.");
    await new Promise(r => setTimeout(r, 1000));
    
    await typeWriter("type4", "This is truly coming from the bottom of my heart. :)");

    // Munculkan footer kecil di akhir teks
    document.getElementById('final-footer').style.display = 'block';
}
    btn.style.top = Math.random() * maxY + 'px';
}

/**
 * Animasi mengetik teks
 */
async function typeWriter(id, text, speed) {
    const el = document.getElementById(id);
    for (let i = 0; i < text.length; i++) {
        el.innerHTML += text.charAt(i);
        await new Promise(res => setTimeout(res, speed));
    }
}

function lokasiReload() { window.location.reload(); }

/**
 * FUNGSI UTAMA - Dengan Web Audio API Amplifier
 */
async function terimaMaaf() {
    const music = document.getElementById('bgMusic');
    const voice = document.getElementById('voiceAI');
    const scene = document.getElementById('dramatic-scene');
    const videoWrapper = document.querySelector('.video-wrapper-landscape');
    const videoEl = document.getElementById('us-video');
    const replayScreen = document.getElementById('replay-screen');

    // --- 1. SETTING AMPLIFIER (WEB AUDIO API) ---
    // Ini buat maksa suara AI George jadi lebih kencang dari batas normal
    const audioCtx = new (window.AudioContext || window.webkitAudioContext)();
    const source = audioCtx.createMediaElementSource(voice);
    const gainNode = audioCtx.createGain();

    // ANGKA SAKTI: 3.5 berarti volume George naik 3.5x lipat (350%)
    gainNode.gain.value = 3.5; 

    source.connect(gainNode);
    gainNode.connect(audioCtx.destination);

    // --- 2. JALANKAN MUSIK ---
    music.currentTime = 210;
    music.volume = 0;
    music.play();
    
    // Musik naik ke 0.7 dan STAY di situ (Tidak menurun)
    let fadeIn = setInterval(() => {
        if (music.volume < 0.7) music.volume += 0.05;
        else clearInterval(fadeIn);
    }, 200);

    // Monitoring Stop
    music.ontimeupdate = () => { if (music.currentTime >= 276) finishEverything(); };

    function finishEverything() {
        scene.classList.add('fade-out-all');
        let fadeOut = setInterval(() => {
            if (music.volume > 0.02) music.volume -= 0.02;
            else {
                music.pause(); clearInterval(fadeOut);
                scene.style.display = 'none';
                replayScreen.style.display = 'flex';
                setTimeout(() => { replayScreen.style.opacity = '1'; }, 100);
            }
        }, 100);
    }

    // --- 3. TRANSISI VISUAL ---
    document.getElementById('content-wrapper').style.opacity = '0';
    document.getElementById('bg-decorations').style.opacity = '0';
    setTimeout(() => {
        document.getElementById('content-wrapper').style.display = 'none';
        document.getElementById('bg-decorations').style.display = 'none';
        scene.style.display = 'flex';
    }, 800);

    // --- 4. MOMEN PUNCAK ---
    await new Promise(r => setTimeout(r, 2500)); 
    videoWrapper.classList.add('show-video');
    videoEl.playbackRate = 0.8; 
    videoEl.play();

    // JALANKAN SUARA AI (Tanpa nurunin volume musik)
    voice.play(); 

    const typoSpeed = 69; 
    await typeWriter("type1", "In the world of literature, there are countless beautiful verses, but none can truly capture how much you mean to me.", typoSpeed);
    await new Promise(r => setTimeout(r, 800));
    await typeWriter("type2", "Just like the lyrics in your photo, 'Lights will guide you home'...", typoSpeed);
    await new Promise(r => setTimeout(r, 500));
    await typeWriter("type3", "I hope I can be one of those lights that always leads you back to where you feel safe and comfortable.", typoSpeed);
    await new Promise(r => setTimeout(r, 1000));
    await typeWriter("type4", "This is truly coming from the bottom of my heart. :)", typoSpeed);

    voice.onended = () => {
        document.getElementById('final-footer').style.display = 'block';
    };
}

